<!--
  ...
  Instead the dock is five separate SVGs, each carrying its own slice of the
  glass bar and a matching blurred crop of the wallpaper. Set flush against each
  other they join into one bar, and each is a real anchor.

  The tags below MUST stay on one line with no whitespace between them.
  Whitespace between inline elements renders as a gap and the bar breaks apart.
